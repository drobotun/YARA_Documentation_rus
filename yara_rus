...
***
**Важно!**: Этот модуль зависит от библиотеки OpenSSL. Пожалуйста, обратитесь к Главе 1. ["Компиляция и установка YARA"](#ch_1) для получения информации о том, как встроить OpenSSL-зависимые функции в YARA.
...
Начиная с YARA версии 3.0 вы можете расширить ее возможности. Это возможно с помощью модулей, которые вы можете использовать для определения структур данных и функций, которые впоследствии можно будет применить в ваших правилах. Вы можете увидеть некоторые примеры того, что может делать модуль, в Главе 3 [«Использование модулей»](#ch_3).
...
Функции YARA могут быть использованы в Python-скриптах с помощью библиотеки `yara-python`. Как только библиотека будет построена и установлена, как описано в Главе 1 ["Компиляция и установка YARA"](#ch_1), вы получите доступ к полному потенциалу YARA из ваших скриптов Python. Первый шаг-импорт библиотеки YARA:
